    ...
    <div className="mb-3">
      ...
      <div className="h-2 rounded-full" style={{ backgroundColor: 'var(--bg-elevated)' }}>
        <div
          ...
          style={{ width: `${value}%`, backgroundColor: 'var(--accent-hover)' }}
        />
      </div>
    </div>
  );
...
function CollapsibleSection({
  ...
  return (
    <div className="border rounded-lg overflow-hidden" style={{ borderColor: 'var(--border)' }}>
      ...
      {open && (
        <div className="px-5 py-4 border-t" style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border)' }}>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border)', border: '1px solid rgba(255,255,255,0.06)' }}
      >
        <button
          onClick={onCancel}
        ...
        <h3 className="mb-2" style={{ fontSize: '1.0625rem', fontWeight: 600, color: 'var(--text-primary)' }}>
          {title}
        </h3>
...
export default function CandidateProfile() {
  ...
  const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
    ...
    hired: { label: 'Advanced', color: 'var(--success)', bg: 'var(--success-bg)' },
  };
        ...
        <div className="flex flex-col lg:flex-row gap-5">
          ...
          <div className="w-full lg:w-80 flex-shrink-0 space-y-4">
            ...
            >
              <div className="flex flex-col items-center mb-5">
                ...
                <h2
                  ...
                  style={{ fontSize: '1.0625rem', fontWeight: 600, color: 'var(--text-primary)' }}
                >
                  {candidate.name}
                </h2>
                <p className="text-sm mt-0.5" style={{ color: 'var(--text-secondary)' }}>{candidate.role}</p>
                {/* Status badge */}
                <span
              ...
              <div className="space-y-2.5">
                <div className="flex items-center gap-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}>
                  <Mail size={13} style={{ color: 'var(--text-secondary)', opacity: 0.6 }} className="flex-shrink-0" />
                  {candidate.email}
                </div>
                <div className="flex items-center gap-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}>
                  <Phone size={13} style={{ color: 'var(--text-secondary)', opacity: 0.6 }} className="flex-shrink-0" />
                  {candidate.phone}
                </div>
                <div className="flex items-center gap-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}>
                  <Calendar size={13} style={{ color: 'var(--text-secondary)', opacity: 0.6 }} className="flex-shrink-0" />
            ...
            <div
              ...
              style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border)', boxShadow: '0 1px 3px rgba(0,0,0,0.3)' }}
            >
              <h3 className="mb-3" style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)' }}>
                Skills from CV
              </h3>
              ...
              <div className="space-y-3">
                {candidate.education.map((e, i) => (
                  <div key={i} className="flex gap-3">
                    ...
                    <div className="pb-3">
                      <p className="text-xs" style={{ color: 'var(--text-secondary)', opacity: 0.6 }}>{e.year}</p>
                ...
                {candidate.experience.map((e, i) => (
                  <div key={i} className="flex gap-3">
                    ...
                    <div className="pb-3">
                      ...
                      <p className="text-sm mb-1" style={{ color: 'var(--text-secondary)' }}>{e.company}</p>
                      <p className="text-xs leading-relaxed" style={{ color: 'var(--text-secondary)', opacity: 0.75 }}>{e.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {candidateStatus !== 'hired' && candidateStatus !== 'rejected' && (
                <>
                  ...
                  <button
                    ...
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.backgroundColor = 'var(--error-bg)'; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.backgroundColor = 'transparent'; }}
                  >
                    Reject
                  </button>
